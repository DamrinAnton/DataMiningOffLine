...
namespace DM.DecisionTree
{
    public class RandomForest
    {
        ...
        public TreeNode MountTree(List<OneRow> samples, Dictionary<int, string> parameterNames, int defectParameterID, int curveCount)
        {
            int parametersCount = parameterNames.Count;
            Attribute[] attributes = new Attribute[parametersCount];
            attributes = CreateAttribute(samples, parametersCount, parameterNames, curveCount, defectParameterID);
            TreeNode root = IMountTree(samples, attributes, parameterNames, defectParameterID, curveCount, 1);
            return root;
        }
