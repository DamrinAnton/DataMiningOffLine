...
namespace Hexagonal
{
    ...
    public class Board
    {
        ...
        private void Initialize(int width, int height, int side, Hexagonal.HexOrientation orientation, int xOffset, int yOffset)
        {
            ...
            float h = Hexagonal.MyMath.CalculateH(side); // short side
            float r = Hexagonal.MyMath.CalculateR(side); // long side
        ...
        public Hex FindHexMouseClick(int x, int y)
        {
            ...
            if (PointInBoardRectangle(x, y))
            {
                for (int i = 0; i < hexes.GetLength(0); i++)
                {
                    ...
                    {
                        if (MyMath.InsidePolygon(hexes[i, j].Points, 6, new System.Drawing.PointF(x, y)))
                        {
                            target = hexes[i, j];
                            break;
                        }
